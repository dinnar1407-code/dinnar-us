      ...
      <section className="bg-navy-500 text-white py-20">
        <div className="container-page">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: lang === "zh" ? "光学" : "Optics", body: lang === "zh" ? "定制远心镜头、多光谱同轴照明和专利偏振片堆叠，按SKU调校。自研设计，自主加工。" : "Custom telecentric lenses, multi-spectral coaxial lighting, and proprietary polariser stacks tuned per SKU. Designed in-house, ground in-house.", icon: "🔬" },
              { title: lang === "zh" ? "机械" : "Mechanics", body: lang === "zh" ? "花岗岩平台、精密直线模组和隔振工作单元，专为8年使用寿命的亚微米级重复精度而设计。" : "Granite stages, precision linear modules, and vibration-isolated work cells engineered for sub-micron repeatability over 8-year service life.", icon: "⚡" },
              { title: lang === "zh" ? "电子" : "Electronics", body: lang === "zh" ? "ACI-S1000 基板、FPGA 图像采集卡和 EtherCAT 运动控制器。每块 PCBA 均由鼎纳自研自测。" : "ACI-S1000 baseboards, FPGA imaging cards, and EtherCAT motion controllers. Every PCBA is built and tested by Dinnar.", icon: "💻" },
            ].map((item, i) => (
              <div key={i} className="bg-white/5 backdrop-blur-sm rounded-xl p-8 border border-white/10 hover:border-brand-500/30 transition-all">
